(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        form.addEventListener("submit", (async function(e) {
            ...
            try {
                ...
                } else button.textContent = "Error in sending";
            } catch (error) {
                button.textContent = "Server unavailable";
            }
            setTimeout((() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        const penIcon = document.querySelector(".menu__con");
        const sectionMap = {
            Home: document.querySelector(".hero"),
            About: document.querySelector(".about"),
            Catalog: document.querySelector(".catalog"),
            Shop: document.querySelector(".save"),
            Blog: document.querySelector(".blog"),
            Subscribe: document.querySelector(".subscribe")
        };
        const sectionValues = Object.values(sectionMap);
        ...
        function updateActiveMenu() {
            ...
            sectionValues.forEach(((section, i) => {
                ...
                if (rect.top <= window.innerHeight / 2 && rect.bottom >= window.innerHeight / 2) activeIndex = i;
            }));
            menuItems.forEach(((item, i) => {
                item.classList.toggle("active", i === activeIndex);
                if (i === activeIndex) currentActiveItem = item;
            }));
            const activeLink = document.querySelector(".menu__item.active .menu__link");
            if (activeLink && penIcon) {
                const linkRect = activeLink.getBoundingClientRect();
                const menuRect = document.querySelector(".menu__body").getBoundingClientRect();
                const offsetLeft = linkRect.left + linkRect.width / 2 - menuRect.left;
                penIcon.style.left = `${offsetLeft}px`;
            }
        }
        updateActiveMenu();
        window.addEventListener("scroll", updateActiveMenu);
        penIcon.addEventListener("mouseenter", (() => {
            if (currentActiveItem) currentActiveItem.classList.add("hover-by-pen");
        }));
        penIcon.addEventListener("mouseleave", (() => {
            if (currentActiveItem) currentActiveItem.classList.remove("hover-by-pen");
        }));
        function updateActiveMenu() {
            let activeIndex = 0;
            sectionValues.forEach(((section, i) => {
            ...
            if (!penIcon || !currentActiveItem) return;
            const isMobile = window.matchMedia("(max-width: 768px)").matches;
            const link = currentActiveItem.querySelector(".menu__link");
            if (!link) return;
            const linkRect = link.getBoundingClientRect();
            const menuRect = document.querySelector(".menu__inner").getBoundingClientRect();
            if (isMobile) {
                const offsetTop = linkRect.bottom - menuRect.top;
                penIcon.style.left = "0";
                penIcon.style.top = `${offsetTop + 8}px`;
                penIcon.style.transform = "none";
            } else {
                const offsetLeft = linkRect.left + linkRect.width / 2 - menuRect.left;
                penIcon.style.left = `${offsetLeft}px`;
                penIcon.style.top = "";
                penIcon.style.transform = "translateX(-50%)";
            }
        }
    }));
    let swiperInstance;
    ...
    function renderCatalog(categoryKey) {
        ...
        setTimeout((() => {
            ...
            initSliders();
            catalogContent.classList.remove("is-hidden");
        }), 200);
    }
    ...
    if (document.querySelector(".blog__slider")) new swiper_core_Swiper(".blog__slider", {
        ...
        navigation: {
            prevEl: ".blog__arrow-left",
            nextEl: ".blog__arrow-right"
        },
        on: {}
    });
    window.addEventListener("load", (function(e) {
